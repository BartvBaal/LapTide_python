"""Solve ODE defining the Associated Legendre Polynomails:
  ...
  (1 - x^2) \diffrac{f}{x} = \left( \lambda (1 - x^2) - m^2 \right) f + 2 \alpha x g
...
class ODE_t:
    ...
    def __call__ (self, t, y):
        sinsq = 1. - t*t
        twoax = 2. * self.alpha * t
        dy0dt = (twoax * y[0] - y[1] ) / sinsq
        dy1dt = self.lam * y[0] - (self.msq * y[0] - twoax * y[1]) / sinsq 
        return [dy0dt, dy1dt]
